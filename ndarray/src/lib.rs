...
fn compute_index(indices: &[usize], strides: &[usize], base_offset: usize) -> usize {
...
fn compute_stride(shape: &[usize]) -> NdArrayIndex {
    shape.iter()
        .rev()
        .scan(1, |acc, x| {
            let tmp = *acc;
...
fn compute_reshape_strides(old_shape: &[usize], old_stride: &[usize], reshape: &[usize]) -> Result<NdArrayIndex, NdArrayError> {
...
impl <T> From<Scalar<T>> for NdArray<T> {
    fn from(data: Scalar<T>) -> Self {
        NdArray::new_array(Box::new([data.0]), index![1], index![1], 0)
    }
}
